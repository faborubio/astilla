...
#
# Uso (folder-aware, jul 2026: todo en artifacts/shorts/<nombre>/):
...
# (--audio/--clips-dir se pueden pasar para override manual)
#
# Tip: mirar <n>/palabras.json para elegir los cortes en los limites de beat.
import argparse
import sys
...
def main() -> None:
    ...
    clips = {i: clips_dir / f"escena_{i:02d}.mp4" for i in range(len(escenas))}
    faltan = [i for i, p in clips.items() if not p.exists()]
    if faltan:
        sys.exit(f"faltan clips: {faltan} en {clips_dir}")
